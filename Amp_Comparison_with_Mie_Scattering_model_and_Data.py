...
#  ---------------------------------------------------------------------------------------------------------------------
# ------------------------------------------------------ HST Data ------------------------------------------------------
# Converting julian into hour and since hst data has gap data wee fill in the gaps with numpy linespace
...
text_title = r"$\bf{" + "Figure" + "}$" +" "+ r"$\bf{" + '3:' + "}$" "Observation of HST and Spitzer"
...
plt.xlim(0,9)
plt.ylim(0.97,1.06)
plt.savefig("/Users/melaniapena/Rsrch/code/2017_fall_semester/data/Int_Amp_Dec_21_17/HST_SPTZ_fit_and_ori_data.png",dpi=600, additional_artists=art, bbox_inches="tight")
plt.show()
# ----------------------------------------------------------------------------------------------------------------------
# ------------------------------------ Overlap plot region------------------------------------------
...
# what we want to be able to compare the amplitudes only with within regions that overlap.
...
# Creates an array just for the HST data (used for the forloop when comparing the amplitude with a Mie Scattering Model.)
Hst_bands =([amp_J_HST, amp_H_HST, amp_W_HST])
hst_band_average = np.average(Hst_bands)
print "HST Amp Average: ", hst_band_average,'\n'
...
#  using the amount of parameters (does not include first parameter) from each fourier series def
k_twoMode = 8.
...
for file in os.listdir(file_dir_Int_models):
    ...
    # graph = plt.plot(Wavelength_File,Extraction_File)
    name_of_file = os.path.splitext(os.path.basename(file))[0]
    two = 2
    # chi_squared = sum(((Amp_of_HST_SPTZ - model_amp_atdata) ** two) / yerr_vector ** two)
    ...
    # text_title = r"$\bf{" + "Figure" + "}$" + "" + r"$\bf{" + '8:' + "}$" "Amp Model Comparison with SPTZ/HST Amp"
